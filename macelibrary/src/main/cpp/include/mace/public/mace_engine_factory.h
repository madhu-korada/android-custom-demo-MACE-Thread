...
std::map<std::string, int> model_name_map {
  std::make_pair("mnist_keras", 0),
  std::make_pair("mnist_keras_network_2", 1),
};
}  // namespace
...
__attribute__((deprecated)) MaceStatus CreateMaceEngineFromCode(
  ...
  switch (model_name_map[model_name]) {
    case 0: {
      multi_net_def = mace::mnist_keras::CreateMultiNet();
      engine->reset(new mace::MaceEngine(config));
      model_data = mace::mnist_keras::LoadModelData();
      const int64_t model_size = mace::mnist_keras::GetModelSize();
    ...
    case 1: {
      multi_net_def = mace::mnist_keras_network_2::CreateMultiNet();
      engine->reset(new mace::MaceEngine(config));
      model_data = mace::mnist_keras_network_2::LoadModelData();
      const int64_t model_size = mace::mnist_keras_network_2::GetModelSize();
      bool model_data_unused = false;
      status = (*engine)->Init(multi_net_def.get(), input_nodes, output_nodes,
                               model_data, model_size, &model_data_unused,
                               tutor);
