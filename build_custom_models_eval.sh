...
ANDROID_DEMO_DIR=examples/android-custom-demo-MACE/
...
python /home/hitech/aware/mace/tools/converter.py convert --config=examples/android-custom-demo-MACE/cifar_models_eval.yml --target_abis=$TARGET_ABI
...
cp -rf build/$MODEL_NAME/include/mace/public/*.h $INCLUDE_DIR/mace/public/
cp -rf build/$MODEL_NAME/model $LIBRARY_DIR
...
#### It copies the libmace.so to jniLibs folder
# if [ $MACE_LINK_TYPE == "dynamic" ]; then
#   mkdir -p $JNILIBS_DIR
